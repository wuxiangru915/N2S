"""
Simple end-to-end tests for N2S agents.
...
def create_agent(llm_service, sql_runner):
    ...
    from n2s.integrations.local.file_system import LocalFileSystem
    from n2s.integrations.local.agent_memory import DemoAgentMemory
    from n2s.tools.agent_memory import (
...
async def check_agent_top_artist(agent, expected_artist="Iron Maiden"):
...
async def test_anthropic_top_artist(chinook_db):
    ...
    await check_agent_top_artist(agent)
...
#     from n2s.integrations.openai import OpenAIResponsesService
