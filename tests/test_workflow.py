...
from n2s.core.workflow.default import DefaultWorkflowHandler
...
from n2s.core.storage import Conversation
from n2s.core.tool import ToolContext
from n2s.capabilities.agent_memory import ToolMemory, TextMemory
from n2s.integrations.local.agent_memory import DemoAgentMemory
from n2s.core.rich_component import ComponentType
...
class TestStarterUI:
    ...
    async def test_starter_ui_single_component(
        ...
        class MockToolRegistryWithSQL:
            async def get_schemas(self, user):
                from n2s.core.tool import ToolSchema
    ...
    async def test_starter_ui_user_view(
        ...
        assert "欢迎使用 N2S" in rich_text.content
    ...
    async def test_starter_ui_admin_view(
        ...
        assert "🔒 管理员视图" in card.content  # Should show admin badge
        assert "Setup:" in card.content
        # Admin view should include memory management info
        assert "记忆管理" in card.content
