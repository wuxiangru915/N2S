...
Thank you for your interest in contributing to N2S! This guide will help you get started with contributing to the N2S codebase.
...
3. Add the upstream repository:
   ```bash
   git remote add upstream https://github.com/n2s-ai/n2s.git
   ```
...
```bash
# Check formatting
ruff format --check src/n2s/ tests/

# Apply formatting
ruff format src/n2s/ tests/

# Run linting
ruff check src/n2s/ tests/
```
...
- `test_ingest/` - Data ingestion module tests
...
N2S is built around several key abstractions:

#### 1. **Agent** (`n2s.core.agent`)
The main orchestrator that coordinates tools, memory, and LLM interactions.

#### 2. **Tools** (`n2s.tools`, `n2s.core.tool`)
...
1. **Create the tool class** in `src/n2s/tools/`:
...
1. **Implement SqlRunner** in `src/n2s/integrations/mydb/`:
...
1. **Implement LlmService** in `src/n2s/integrations/myllm/`:

```python
from n2s.core.llm.base import LlmService
from n2s.core.llm.models import LlmRequest, LlmResponse, LlmStreamChunk
...
1. **Implement AgentMemory** in `src/n2s/integrations/mystore/`:

```python
from n2s.capabilities.agent_memory import (
    AgentMemory,
    ToolMemory,
...
- The `LegacyVannaAdapter` bridges legacy code with N2S
- Add tests to `tests/test_legacy_adapter.py`
- See `src/n2s/legacy/adapter.py` for examples

---

## Getting Help

- **GitHub Issues**: https://github.com/n2s-ai/n2s/issues
- **Discussions**: https://github.com/n2s-ai/n2s/discussions

---

## License

By contributing to N2S, you agree that your contributions will be licensed under the MIT License.

---

Thank you for contributing to N2S! 🎉
