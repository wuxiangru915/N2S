#!/usr/bin/env python3
"""
Comprehensive test backend for n2s-webcomponent validation.
...
from n2s.core.rich_component import RichComponent, ComponentLifecycle
from n2s.components.rich import (
    RichTextComponent,
    StatusCardComponent,
...
from n2s.servers.base.models import ChatStreamChunk

# Request/Response models
class ChatRequest(BaseModel):
    """Chat request matching n2s API."""
    message: str
...
Watch the sidebar checklist as components render! ➡️""",
    ...
    # NOTE: Table, Container, and CodeBlock components are defined in n2s Python package
    # but NOT supported by the webcomponent (no renderers). Skipping these tests.
    # These are candidates for removal from the n2s package.
...
# FastAPI app
app = FastAPI(title="N2S Webcomponent Test Backend")
...
async def root():
    ...
    return {
        "message": "N2S Webcomponent Test Backend",
        "mode": test_state["mode"],
        "endpoints": {
            "chat": "POST /api/n2s/v2/chat_sse",
            "health": "GET /health",
        },
    }
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Test backend for n2s-webcomponent")
    parser.add_argument(
        "--mode",
