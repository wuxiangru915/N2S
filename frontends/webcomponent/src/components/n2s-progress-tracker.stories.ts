...
import './n2s-progress-tracker';

const meta: Meta = {
  title: 'Components/N2sProgressTracker',
  component: 'n2s-progress-tracker',
  parameters: {
    layout: 'centered',
    backgrounds: {
      default: 'light',
...
export const Empty: Story = {
  ...
  render: (args) => html`
    <div style="width: 350px; height: 400px;">
      <n2s-progress-tracker .title=${args.title}></n2s-progress-tracker>
    </div>
  `,
};
...
export const WithTasks: Story = {
  ...
  render: (args) => {
    setTimeout(() => {
      const tracker = document.querySelector('n2s-progress-tracker') as any;
      if (tracker) {
    ...
    return html`
      <div style="width: 350px; height: 400px; background: ${args.theme === 'light' ? '#ffffff' : 'rgb(11, 15, 25)'}; padding: 20px;">
        <n2s-progress-tracker .title=${args.title} theme=${args.theme}></n2s-progress-tracker>
      </div>
    `;
  },
};
...
export const LiveDemo: Story = {
  ...
  render: (args) => {
    ...
    const runDemo = () => {
      if (!tracker) {
        tracker = document.querySelector('n2s-progress-tracker');
        if (!tracker) {
