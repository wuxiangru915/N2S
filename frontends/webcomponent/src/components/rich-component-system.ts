/**
 * Rich Component System for N2S Agent
 *
...
const RICH_COMPONENT_STYLE_ATTR = 'data-n2s-rich-component-styles';
...
export class TaskListComponentRenderer extends BaseComponentRenderer {
  ...
  private getStatusIcon(status: string): string {
    const labels: Record<string, string> = {
      completed: '<span class="status-icon-label">[已完成]</span>',
      running: '<span class="status-icon-label">[进行中]</span>',
      failed: '<span class="status-icon-label">[失败]</span>',
      pending: '<span class="status-icon-label">[待处理]</span>'
    };
...
export class NotificationComponentRenderer extends BaseComponentRenderer {
  ...
  private getLevelIcon(level: string): string {
    const labels: Record<string, string> = {
      success: '<span class="status-icon-label">[成功]</span>',
      warning: '<span class="status-icon-label">[警告]</span>',
      error: '<span class="status-icon-label">[错误]</span>',
      info: '<span class="status-icon-label">[信息]</span>'
    };
...
export class StatusIndicatorComponentRenderer extends BaseComponentRenderer {
  ...
  private getStatusIcon(status: string): string {
    const labels: Record<string, string> = {
      loading: '<span class="status-icon-label">[加载中]</span>',
      success: '<span class="status-icon-label">[成功]</span>',
      warning: '<span class="status-icon-label">[警告]</span>',
      error: '<span class="status-icon-label">[错误]</span>',
      info: '<span class="status-icon-label">[信息]</span>'
    ...
    if (searchable || exportable || filterable) {
      actionsHTML = `
        <div class="dataframe-actions">
          ...
          ` : ''}
          ${exportable ? `
            <button class="export-btn" title="导出 CSV">导出</button>
          ` : ''}
        </div>
      `;
    }
        ...
        <div class="dataframe-table-container">
          ...
          ${hasMoreRows ? `
            <div class="dataframe-truncated">
              <em>显示前 ${max_rows_displayed} 行，共 ${row_count} 行</em>
            </div>
          ` : ''}
        </div>
      `;
...
export class ButtonComponentRenderer extends BaseComponentRenderer {
  render(component: RichComponent): HTMLElement {
    ...
    let buttonContent = '';
    if (loading) {
      buttonContent = `<span class="button-spinner">[加载中]</span><span class="button-label">${label}</span>`;
    } else if (icon) {
    ...
    if (action && !disabled && !loading) {
      button.addEventListener('click', async () => {
        ...
        // Find n2s-chat component and send message with button action
        const n2sChat = document.querySelector('n2s-chat') as any;
        console.log('   Found n2s-chat:', !!n2sChat);
        ...
        if (n2sChat && typeof n2sChat.sendMessage === 'function') {
          ...
          console.error('   ✗ n2s-chat not found or sendMessage not available');
          // Restore button state if it wasn't originally disabled
          if (!disabled) {
            button.disabled = false;
          }
      ...
      if (buttonConfig.action && !buttonConfig.disabled) {
        button.addEventListener('click', async () => {
          ...
          if (n2sChat && typeof n2sChat.sendMessage === 'function') {
            console.log('   Calling sendMessage...');
            try {
              const success = await n2sChat.sendMessage(buttonConfig.action);
              if (success) {
                console.log('   ✓ Message sent successfully');
              } else {
                console.log('   ✗ Message failed, restoring button state');
...
export class ChartComponentRenderer extends BaseComponentRenderer {
  render(component: RichComponent): HTMLElement {
    ...
    if (plotlyData && Array.isArray(plotlyData) && layout) {
      ...
      if (n2sChat) {
        chartElement.theme = n2sChat.getAttribute('theme') || 'dark';
      }
...
export class ArtifactComponentRenderer extends BaseComponentRenderer {
      ...
      container.innerHTML = `
        <div class="artifact-placeholder">
          <div class="placeholder-content">
            <span class="placeholder-icon">[Artifact]</span>
  ...
  private fireArtifactOpenedEvent(component: RichComponent, trigger: 'created' | 'user-action', container: HTMLElement): boolean {
    ...
    // Fire the event from the container element (should bubble up to n2s-chat)
    container.dispatchEvent(event);

    // Also dispatch directly on the n2s-chat element as backup
    const n2sChat = container.closest('n2s-chat');
    if (n2sChat) {
      console.log('📡 Also dispatching on n2s-chat element');
      n2sChat.dispatchEvent(new CustomEvent('artifact-opened', {
        detail: eventDetail,
        bubbles: true,
        cancelable: true
      }));
...
export class ComponentRegistry {
  ...
  private renderWebComponent(tagName: string, component: RichComponent): HTMLElement {
    ...
    // Set theme to match the parent N2sChat theme
  ...
  private getCurrentTheme(): string {
    // Try to get theme from the parent N2sChat component
    const n2sChat = document.querySelector('n2s-chat');
    if (n2sChat) {
      return n2sChat.getAttribute('theme') || 'dark';
    }
    return 'dark';
  }
...
export class ComponentManager {
  ...
  private triggerScroll(): void {
    // Find the parent n2s-chat component and trigger its scroll method
    const n2sChat = document.querySelector('n2s-chat') as any;
    if (n2sChat && typeof n2sChat.scrollToLastMessage === 'function') {
      // Use requestAnimationFrame to wait for DOM update
      requestAnimationFrame(() => {
        requestAnimationFrame(() => {
          n2sChat.scrollToLastMessage();
        });
      });
    }
  }
  ...
  private updateStatusBar(component: RichComponent): void {
    ...
    // Look for n2s-chat and search within its shadow root
    const n2sChat = document.querySelector('n2s-chat') as any;
    if (n2sChat && n2sChat.shadowRoot) {
      statusBar = n2sChat.shadowRoot.querySelector('n2s-status-bar') as HTMLElement | null;
    }

    // Fallback to document search
    if (!statusBar) {
      statusBar = document.querySelector('n2s-status-bar') as HTMLElement | null;
    }
  ...
  private updateTaskTracker(component: RichComponent): void {
    ...
    const n2sChat = document.querySelector('n2s-chat') as any;
    if (n2sChat && n2sChat.shadowRoot) {
      progressTracker = n2sChat.shadowRoot.querySelector('n2s-progress-tracker');
    }

    // Fallback to document search
    if (!progressTracker) {
      progressTracker = document.querySelector('n2s-progress-tracker');
    }
  ...
  private updateChatInput(component: RichComponent): void {
    ...
    const n2sChat = document.querySelector('n2s-chat') as any;
    if (n2sChat && n2sChat.shadowRoot) {
      chatInput = n2sChat.shadowRoot.querySelector('textarea.message-input, input.message-input');
    }
