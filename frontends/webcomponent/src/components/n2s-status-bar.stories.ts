...
import './n2s-status-bar';

const meta: Meta = {
  title: 'Components/N2sStatusBar',
  component: 'n2s-status-bar',
  parameters: {
    layout: 'centered',
    backgrounds: {
      default: 'light',
...
export const StatusSequence: Story = {
  render: () => {
    ...
    setTimeout(() => {
      statusBar = document.querySelector('n2s-status-bar');
      updateStatus();
    }, 100);

    return html`
      <div style="width: 500px; padding: 20px;">
        <n2s-status-bar status="idle"></n2s-status-bar>
        <div style="margin-top: 10px; color: #999; font-size: 12px;">
