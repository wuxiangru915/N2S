# N2S Webcomponent Comprehensive Test Suite

This test suite validates all component types and update patterns in the n2s-webcomponent before pruning unused code.
...
3. **Remove one piece of cruft**:
   ```bash
   # Example: Remove unused import from n2s-chat.ts
...
**Error**: `ModuleNotFoundError: No module named 'n2s'`

**Solution**: Make sure n2s is in the Python path:
```bash
cd submodule/n2s-webcomponent
python test_backend.py  # Already adds ../../src to sys.path
```
...
1. FastAPI receives POST to `/api/n2s/v2/chat_sse`
...
1. `<n2s-chat>` web component connects to backend
2. Opens SSE connection to `/api/n2s/v2/chat_sse`
...
3. Backend receives message via `/api/n2s/v2/chat_sse` POST
...
5. **Copy to n2s package** - Integrate cleaned webcomponent into n2s Python package
