"""
Agent implementation for the N2S Agent framework.
...
from n2s.core.storage import ConversationStore
from n2s.core.llm import LlmService
from n2s.core.system_prompt import SystemPromptBuilder
from n2s.core.storage import Conversation, Message
from n2s.core.llm import LlmMessage, LlmRequest, LlmResponse
from n2s.core.tool import ToolCall, ToolContext, ToolResult, ToolSchema
from n2s.core.user import User
from n2s.core.registry import ToolRegistry
from n2s.core.system_prompt import DefaultSystemPromptBuilder
from n2s.core.lifecycle import LifecycleHook
from n2s.core.middleware import LlmMiddleware
from n2s.core.workflow import WorkflowHandler, DefaultWorkflowHandler
from n2s.core.recovery import ErrorRecoveryStrategy, RecoveryActionType
from n2s.core.enricher import ToolContextEnricher
from n2s.core.enhancer import LlmContextEnhancer, DefaultLlmContextEnhancer
from n2s.core.filter import ConversationFilter
from n2s.core.observability import ObservabilityProvider
from n2s.core.user.resolver import UserResolver
from n2s.core.user.request_context import RequestContext
from n2s.core.agent.config import UiFeature
from n2s.core.audit import AuditLogger
from n2s.capabilities.agent_memory import AgentMemory

import logging

logger = logging.getLogger(__name__)

logger.info("Loaded n2s.core.agent.agent module")

# UI message localization for demo languages.
_UI_MESSAGES: dict[str, dict[str, str]] = {
    "zh": {
        "Ready": "就绪",
        "Choose an option or type a message": "选择一个选项或输入消息",
        "Ask a question...": "输入问题...",
        "Workflow complete": "工作流完成",
        "Ready for next message": "等待下一条消息",
        "Processing your request...": "正在处理你的请求...",
        "Analyzing query": "正在分析问题",
        "Load conversation context": "加载对话上下文",
        "Response complete": "响应完成",
        "Ask a follow-up question...": "输入后续问题...",
        "Error Processing Message": "处理消息时出错",
        "Error occurred": "发生错误",
        "An unexpected error occurred while processing your message": "处理消息时发生意外错误",
        "Try again...": "重试...",
        "Executing tools...": "正在执行工具...",
        "Tool limit reached": "已达到工具调用上限",
        "Continue the task or ask me something else...": "继续任务或问其他问题...",
    }
}


def _ui_text(text: str, lang: str) -> str:
    """Return localized UI text if a translation exists, otherwise the original."""
    return _UI_MESSAGES.get(lang, {}).get(text, text)
...
class Agent:
    ...
    def __init__(
        ...
        if conversation_store is None:
            from n2s.integrations.local import MemoryConversationStore
        ...
        try:
            ...
            yield UiComponent(  # type: ignore
                rich_component=ChatInputUpdateComponent(
                    placeholder=_ui_text("Try again...", lang), disabled=False
                )
            )
    ...
    async def _send_message(
        ...
        *,
        ...
        if is_starter_request and self.workflow_handler:
            ...
            try:
                ...
                components = await self.workflow_handler.get_starter_ui(
                    self, user, conversation, lang=lang
                )
                ...
                if components:
                    ...
                    yield UiComponent(  # type: ignore
                        rich_component=StatusBarUpdateComponent(
                            status="idle",
                            message=_ui_text("Ready", lang),
                            detail=_ui_text("Choose an option or type a message", lang),
                        )
                    )
                    yield UiComponent(  # type: ignore
                        rich_component=ChatInputUpdateComponent(
        ...
        yield UiComponent(  # type: ignore
            rich_component=StatusBarUpdateComponent(
                status="working",
                message=_ui_text("Processing your request...", lang),
                detail=_ui_text("Analyzing query", lang),
            )
        )
            ...
            try:
                ...
                if workflow_result.should_skip_llm:
                    ...
                    yield UiComponent(  # type: ignore
                        rich_component=StatusBarUpdateComponent(
                            status="idle",
                            message=_ui_text("Workflow complete", lang),
                            detail=_ui_text("Ready for next message", lang),
                        )
                    )
                    yield UiComponent(  # type: ignore
                        rich_component=ChatInputUpdateComponent(
        ...
        # Add initial task
        context_task = Task(
            title=_ui_text("Load conversation context", lang),
        ...
        context = ToolContext(
            ...
            metadata={"ui_features_available": ui_features_available, "lang": lang},
        )
        ...
        while tool_iterations < self.config.max_tool_iterations:
            ...
            if response.is_tool_call():
                ...
                if response.content is not None:
                    ...
                    if has_tool_invocation_message_in_chat:
                        ...
                        yield UiComponent(  # type: ignore
                            rich_component=StatusBarUpdateComponent(
                                status="working",
                                message=_ui_text("Executing tools...", lang),
                ...
                yield UiComponent(  # type: ignore
                    rich_component=StatusBarUpdateComponent(
                        status="idle",
                        message=_ui_text("Response complete", lang),
                        detail=_ui_text("Ready for next message", lang),
                    )
                )
        ...
        if tool_iterations >= self.config.max_tool_iterations:
            ...
            yield UiComponent(  # type: ignore
                rich_component=StatusBarUpdateComponent(
                    status="warning",
                    message=_ui_text("Tool limit reached", lang),
            ...
            warning_message = f"""**Tool Execution Limit Reached**
