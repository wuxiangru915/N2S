...
Usage:
  PYTHONPATH=. python n2s/examples/claude_sqlite_example.py
"""
...
async def main() -> None:
    ...
    from n2s import AgentConfig, Agent
    from n2s.core.registry import ToolRegistry
    from n2s.core.user import CookieEmailUserResolver, RequestContext
    from n2s.integrations.sqlite import SqliteRunner
    from n2s.tools import (
        RunSqlTool,
        VisualizeDataTool,
        LocalFileSystem,
    )
    ...
    if not os.path.exists(database_path):
        ...
        print(
            "Please download it with: curl -o Chinook.sqlite https://n2s.ai/Chinook.sqlite"
        )
        sys.exit(1)
...
def create_demo_agent() -> "Agent":
    ...
    This function is called by the n2s server framework.
    ...
    from n2s import AgentConfig, Agent
    from n2s.core.registry import ToolRegistry
    from n2s.core.user import CookieEmailUserResolver
    from n2s.integrations.sqlite import SqliteRunner
    from n2s.tools import (
        RunSqlTool,
        VisualizeDataTool,
    ...
    if not os.path.exists(database_path):
        raise FileNotFoundError(
            f"Chinook database not found at {database_path}. Please download it from https://n2s.ai/Chinook.sqlite"
        )
