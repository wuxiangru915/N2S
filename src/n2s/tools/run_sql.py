...
from n2s.capabilities.file_system import FileSystem
from n2s.integrations.local import LocalFileSystem


def _sql_ui_text(text: str, lang: str) -> str:
    """Return localized SQL result UI text."""
    translations = {
        "zh": {
            "Query Results": "查询结果",
            "No rows returned": "未返回任何行",
            "Query executed successfully. No rows returned.": "查询执行成功，未返回任何行。",
            "Query executed successfully. {rows_affected} row(s) affected.": "查询执行成功，影响 {rows_affected} 行。",
        }
    }
...
class RunSqlTool(Tool[RunSqlToolArgs]):
    ...
    async def execute(self, context: ToolContext, args: RunSqlToolArgs) -> ToolResult:
        ...
        lang = context.metadata.get("lang", "zh") if context.metadata else "zh"
        try:
                ...
                if df.empty:
                    ...
                    ui_component = UiComponent(
                        rich_component=DataFrameComponent(
                            rows=[],
                            columns=[],
                            title=_sql_ui_text("Query Results", lang),
                            description=_sql_ui_text("No rows returned", lang),
                        ),
                    ...
                    dataframe_component = DataFrameComponent.from_records(
                        ...
                        title=_sql_ui_text("Query Results", lang),
                        description=f"SQL 查询返回 {row_count} 行 {len(columns)} 列",
                    )
